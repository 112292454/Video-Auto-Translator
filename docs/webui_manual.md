...
1. **配置热更新**：每次任务执行都会重新加载当前生效的配置文件（优先 `config/config.yaml`，否则回落 `config/default.yaml`）。修改配置后，重新运行任务即可应用新配置，无需重启 Web 服务。
...
如果问题属于**配置、环境、提示词、LLM 连通性、ffmpeg/ffprobe、Whisper 模型或样例视频探测**，优先进入 `/test` 页面，而不是先重跑任务。

建议最小路径：
1. 打开顶部导航「🧪 测试」进入测试中心
2. 先检查配置是否正确，再做对应的 LLM / 环境自检
3. 确认测试中心结果正常后，再回到任务页重试

任务执行期的失败、进度异常、Playlist 同步失败等问题，再按下面各节继续排查。
