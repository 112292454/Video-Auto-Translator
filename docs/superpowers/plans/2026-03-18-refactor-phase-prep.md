...
## 20. 2026-03-26 统一重构原则

这一节不是记录某个单点改动，而是把当前这轮重构真正依赖的工程原则明确写下来。后续继续推进时，如果局部实现和这里冲突，应优先回到这里重新判断。

### 20.1 从需求出发，而不是从代码形状出发

当前重构的首要目标不是“把大文件拆小”，也不是“把代码改得更像某种模板化架构”，而是：
...
- 测试首先是行为契约，不只是回归护栏
- 文档首先要描述“为什么这个行为重要”，而不是只描述“代码现在怎么写”
- 任何结构性调整，如果不能说明它在行为层面减少了什么风险、澄清了什么边界、提升了什么可维护性，就不应仅仅为了“更整洁”而做

### 20.2 项目形态：单人/小团队优先，不按企业项目过度设计

这个项目的现实假设不是“大团队、多服务、多层审批”的企业软件，而更接近：

- 单人或极少数维护者长期演进
- 用户多为个人
- 需要长期面对外部平台 API、GPU、网络、第三方工具的不稳定性
...
- 为了“未来可能有很多人接手”提前铺复杂框架
- 为了“万一以后会扩很多平台”过度抽象出大量空接口和空层级

### 20.3 两个极端都要避免

#### 不要过度防御

当前项目不适合引入大量企业化补偿框架、命令总线、Saga 编排器、Provider/Factory/Manager 套壳。

原因不是这些模式永远不好，而是：

- 当前复杂度还没有高到值得承担这些结构成本
- 单人维护下，过多间接层会直接拉高理解成本
- 很多问题的真正根源是状态语义不清，而不是“缺少一个更大的框架”

#### 也不要完全不抽象

同样不能继续容忍：

- 巨型控制函数把规划、抓取、裁剪、落库、清理、下游动作全部揉在一起
- 状态语义散落在 route / service / db / uploader 的不同角落
- 远端副作用只有布尔返回值，没有中间态

这类代码虽然“没有抽象”，但维护成本会持续累积，而且会使需求变化时只能继续堆补丁。
...
1. **对跨阶段业务语义做抽象**
   - 例如 satisfied 状态语义
   - 例如 Bilibili 远端副作用恢复状态
   - 例如 playlist sync 的阶段边界

2. **对重复且稳定的编排片段做抽象**
   - 例如 `sync_playlist()` 里的候选规划、fetch 收集、fetch 后裁剪、成员落库、fetch 结果应用
   - 例如 `fix_violation()` 里的违规上下文装载、视频源决策、遮罩渲染

3. **不对尚未稳定的变化点过早抽象**
   - 如果需求和边界还没确定，先保留为显式代码
   - 先通过测试和文档固定行为，再决定是否收成 helper / workflow

一句话：

`先抽稳定语义和重复编排，不抽未来猜测出来的层级`

### 20.5 设计模式的使用原则

当前不是“拒绝设计模式”，而是：

- 只在模式能明确降低理解成本、隔离副作用、澄清边界时使用
- 不为“代码看起来更高级”而套模式

当前已经体现出价值的模式/思路包括：

- Facade：统一 LLM 调用入口
- Workflow：把高层业务流程从 adapter/route 中拉出来
- Thin wrapper / compatibility layer：在不破坏旧调用点的前提下迁移边界
- Stage-oriented decomposition：把大流程拆成可验证的阶段 helper

而当前明确不应优先引入的包括：

- 大而全的 saga framework
- 命令总线式的全仓事件化
- 为所有平台/所有动作一开始就做统一元框架

## 21. 2026-03-26 当前总体状态
...
- `Phase B`：状态语义收口
- `Phase C`：主控制链唯一化
- `Phase D`：业务 workflow 第一轮归位
- `Phase E`：技术子域第一轮收口

这些阶段不是“只有测试”，而是已经包含实际代码行为调整：

- Web 去掉重复旧 API 和启动副作用
- `SKIPPED` 纳入 satisfied 状态语义
- `cli process` 与 `scheduler` 统一到共享运行时
- `WatchService` / Bilibili 高层 workflow 从不合适的调用层移出
- LLM facade / 媒体基础操作 / 字幕 codec 完成第一轮收口

### 21.2 review 提出的真实回归已修复

之前 review 暴露的几类真实问题已经进入分支历史：

- job 查找不能只扫最近固定窗口
- skipped 语义要扩散到剩余状态视图
- season sync 兼容 wrapper 不能被绕过

这说明当前流程里，测试与 review 都不是装饰，而是真正在修正我们对“当前已完成状态”的判断。
...
- `replace_video_with_recovery`
- `sync_season_episode_titles_with_recovery`
- `fix_violation_with_recovery`

并且：

- `replace_video()` 已拆出上下文加载 / 文件上传 / edit 提交
- `fix_violation()` 已拆出违规上下文装载 / 视频源决策 / 遮罩渲染
- `sync_season_episode_titles()` 已有 playlist 级恢复状态记录

#### playlist_service.sync_playlist 阶段化

当前 `sync_playlist()` 已经开始从“巨型事务脚本”收成显式阶段：

1. `_plan_sync_candidates()`
2. `_collect_fetch_results()`
3. `_prune_sync_candidates_after_fetch()`
4. `_persist_sync_members()`
5. `_apply_fetch_results()`

这还不是最终形态，但已经把最容易混杂的前半段编排拆出了稳定切口。

## 22. 接下来可能还要做什么

### 22.1 `playlist_service.sync_playlist()` 继续阶段化

最可能继续推进的方向：

- 把 playlist 最终收尾更新与 upload_order_index 分配再分开
- 进一步明确“fetch 成功后的下游动作”与“成员真值落库”之间的边界
- 再决定是否把 `refresh_videos()` / `retranslate_videos()` 中重复的部分逐步对齐

### 22.2 `fix_violation` 更细粒度恢复

虽然已有第一层恢复状态和内部切口，但更细阶段仍未完全落地：

- `submit_replacement`
- `post_submit_followups`
- 审核等待与后续 `resync_video_info / add_to_season`

这里仍需要继续从需求和恢复语义出发，而不是只继续拆 helper。

### 22.3 `watch_sessions/watch_rounds` 与 `web_jobs`

这仍然是一个中长期统一问题，但优先级低于前两项。
...
1. 先问这一步是在保护什么需求行为，再决定怎么拆。
2. 优先让测试名称表达行为，而不是只表达实现细节。
3. 对单人项目保持克制：可读性和可追踪性优先于“模式完整性”。
4. 对远端副作用保持保守：宁可少做自动化补偿，也不要伪装成已恢复。
5. 文档要持续维护，不能让“当前判断”再次散落到对话和提交历史里。
