...
- 从空目录克隆到完整跑通的部署流程，见 [从零启动 VAT](docs/from_scratch_quickstart.md)
...
日常编辑请优先使用 [`config/config.yaml`](config/config.yaml)；[`config/default.yaml`](config/default.yaml) 仅作为完整字段与注释参考。

如果你是在一台新机器或空目录中第一次部署，建议按 [从零启动 VAT](docs/from_scratch_quickstart.md) 的顺序复现完整流程。
...
如果是首次部署，或怀疑配置 / 环境有问题，优先进入 WebUI 的测试中心（`/test`）做自检。
...
其中 `vat/cli/`、`vat/web/` 与 `vat/services/` 的模块文档可直接查看：[CLI 模块文档](vat/cli/readme.md)、[Web 模块文档](vat/web/readme.md)、[Services 模块文档](vat/services/readme.md)。
更深入模块文档入口可直接查看：[ASR 模块文档](vat/asr/readme.md)、[Embedder 模块文档](vat/embedder/readme.md)、[Translator 模块文档](vat/translator/readme.md)、[Uploaders 模块文档](vat/uploaders/readme.md)、[Pipeline 模块文档](vat/pipeline/readme.md)、[LLM 模块文档](vat/llm/readme.md)、[Downloaders 模块文档](vat/downloaders/readme.md)、[Subtitle Utils 模块文档](vat/subtitle_utils/readme.md)、[Utils 模块文档](vat/utils/readme.md)。
...
多 GPU 环境下的任务分配策略说明，参见归档文档 [GPU 分配规范（仅供参考）](docs/archive/gpu_allocation_spec.md)。
...
| 文档 | 内容 |
|------|------|
| [从零启动 VAT](docs/from_scratch_quickstart.md) | 从空目录克隆、安装、配置、自检到跑通本地视频/上传的完整流程 |
...
| [GPU 分配规范（归档）](docs/archive/gpu_allocation_spec.md) | 多 GPU 调度策略 |
...
| [多源下载设计（归档）](docs/archive/design_multi_source_download.md) | 本地文件/直链/YouTube 多源下载架构设计 |
